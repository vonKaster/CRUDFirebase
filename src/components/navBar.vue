<template>
  <div>
    <v-app-bar class="marginBottom" app fixed color="white" dark>
      <v-toolbar-title>
      ...
      >
        <template v-slot:activator="{ on }">
          <v-btn color="indigo" v-on="on" icon>
      ...
      <v-toolbar-items v-if="!showMenu && $vuetify.breakpoint.mdAndUp">
        <v-btn color="indigo" text v-if="userExists" :to="{ name: 'home' }"
          ><v-icon class="mr-1">mdi-home</v-icon>Inicio</v-btn
        >
        <v-btn color="indigo" text v-if="!userExists" :to="{ name: 'login' }"
